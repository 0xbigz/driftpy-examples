...
ORDERBOOK_URL_DEVNET = "https://master.dlob.drift.trade/orders/json"
...
def get_order_book(market_index):
    """
        returns orderbook snapshot for market_index
    
    """
    try:
        r = requests.get(ORDERBOOK_URL_DEVNET).json()
    except:
        raise Exception("Error retrieving orderbook")
    
    market_to_oracle_map = pd.DataFrame(r['oracles']).set_index()
